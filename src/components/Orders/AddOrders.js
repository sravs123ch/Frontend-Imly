...
//   const { OrderID, setOrderID, orderDate, setOrderDate } = useContext(IdContext);
...
import 'aos/dist/aos.css';
import StatusBadge from './Statuses';
import { CREATE_ORDERS, COUNTRIES_API,GETALLUSERS_API,GET_ALL_ORDERS,STATES_API, CITIES_API, SEARCH_CUSTOMERS, GETALLCUSTOMERS_API, GETALLSTORES_API ,ORDERBYCUSTOMERID_API} from "../../Constants/apiRoutes";
import {
  MdArrowBackIosNew,
  MdOutlineArrowForwardIos,
} from "react-icons/md";
...
//       // Display success toast
//       toast.success(isUpdate ? "Order updated successfully!" : "Order created successfully!");

//       // After order creation or update, fetch order details by generated ID
//       if (generatedId) {
//           // Update order details with the new ID
//           const updatedOrderDetails = {
//               ...orderDetails,
//               OrderID: generatedId,
//           };
...
//           setOrderDetails(updatedOrderDetails); // Update order details with the new ID
//           setOrderIdDetails({ order: updatedOrderDetails }); // Set state for edit mode
//           setIsEditMode(true); // Enable edit mode

//           // Fetch the order details using the generated ID
//           fetch(`https://imlystudios-backend-mqg4.onrender.com/api/orders/getOrderById/${generatedId}`)
//             .then((response) => response.json())
//             .then((data) => {
//               if (data?.order) {
...
const handleSubmit = async (e) => {
  ...
  try {
      ...
      if (generatedId) {
          ...
          // Fetch the order details using the generated ID
          fetch(`https://imly-b2y.onrender.com/api/orders/getOrderById/${generatedId}`)
            .then((response) => response.json())
            .then((data) => {
              if (data?.order) {
                setOrderDetails(data.order);  // Update order details from fetched data
...
//   // Determine if this is an update or a new order
//   const isUpdate = !!orderDetails.OrderID; // Simplified boolean check
...
//   try {
//       // Send API request to create or update the order
//       const response = await axios.post(
//           'https://imly-b2y.onrender.com/api/orders/createOrderOrUpdate',
//           data,
//           {
...
//       const responsedata = response.data;
//       const generatedId = responsedata.OrderID;
//       console.log("Generated Order ID:", generatedId); // Log the generated ID
...
//       // Get the first order from the updated orders list and use its OrderID
//       if ( data && data.length > 0) {
//         const firstOrder = data[0]; // Take the first order from the updated list
//         const orderID = firstOrder.OrderID;
       
//         // Fetch the order details using the OrderID from the first order
//         fetch(`https://imlystudios-backend-mqg4.onrender.com/api/orders/getOrderById/${generatedId}`)
//           .then((response) => response.json())
//           .then((data) => {
//             if (data?.order) {
//               setOrderDetails(data.order); // Update order details with fetched data
//               console.log("Order details fetched and updated:", data.order);
//             }
//           })
//           .catch((error) => {
